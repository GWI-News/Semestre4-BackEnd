...
namespace GwiNews.Domain.Entities
{
    public class Formation
    {
        ...
        public Formation(Guid? id, string? name, string? institution, DateTime? startDate, DateTime? endDate, string? activity1, string? activity2, string? activity3, ReaderUser? readerUser)
        {
            if (id == null || id == Guid.Empty)
            {
                throw new DomainExceptionValidation("Id deve ser um GUID válido e não pode ser vazio ou nulo.");
            }
            ValidateDomain(name, institution, startDate, endDate, activity1, activity2, activity3, readerUser);
            Id = id;
        }

        public Formation(string? name, string? institution, DateTime? startDate, DateTime? endDate, string? activity1, string? activity2, string? activity3, ReaderUser? readerUser)
        {
            ValidateDomain(name, institution, startDate, endDate, activity1, activity2, activity3, readerUser);
        }

        private void ValidateDomain(string? name, string? institution, DateTime? startDate, DateTime? endDate, string? activity1, string? activity2, string? activity3, ReaderUser? readerUser)
        {
